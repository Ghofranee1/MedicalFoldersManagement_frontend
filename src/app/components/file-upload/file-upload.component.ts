...
export class FileUploadComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dossierId = this.route.snapshot.paramMap.get('dossierId') ?
      parseInt(this.route.snapshot.paramMap.get('dossierId')!) : null;
    ...
    if (this.dossierId) {
      this.uploadForm.patchValue({ dossierId: this.dossierId });
      this.uploadForm.get('dossierId')?.disable();
    }
  }
  ...
  onSubmit(): void {
    ...
    const formData = this.uploadForm.getRawValue(); // Use getRawValue to include disabled fields
    ...
    this.uploadFilesSequentially(0, formData);
  }
  ...
  private uploadFilesSequentially(index: number, formData: any): void {
    ...
    uploadFormData.append('File', file); // Note: capital 'F' to match C# property

    // Ensure dossierId is valid before converting to string
    const dossierId = formData.dossierId || this.dossierId;
    if (!dossierId) {
      this.error = 'Dossier ID is required for file upload.';
      this.uploading = false;
      return;
    }
    uploadFormData.append('DossierId', dossierId.toString());
